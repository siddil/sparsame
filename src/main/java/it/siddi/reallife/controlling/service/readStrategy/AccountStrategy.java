...
public class AccountStrategy implements ReadStrategy {

    private Log logger = LogFactory.getLog(AccountStrategy.class);

    @Override
    public List<Movement> readDoc(String[] contents) {
        List<Movement> movements = Lists.newArrayList();
        SimpleDateFormat formatter = new SimpleDateFormat("dd.MM.yyyy");
        String[] relevant = Arrays.copyOfRange(contents, 8, contents.length);
        for (String line:relevant) {
            String[] content = line.split(";");
            String date = content[1].replace("\"","");
            try {
                Date dateMovement=formatter.parse(date);
                String reason = content[3].replace("\"","");
                String amountAsString = content[7].replace("\"", "").replace(".","").replace(',','.');
